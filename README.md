...
Note that this is still is work in progress and not all features has been fully tested. Lot of effort went into getting the emulator to shift data fast through the SPI interface to the LCD to make it run as fast as possible (80MHz clock with short leads). The CPU (in a separate task) is just as fast as transfering data with DMA and has lower latency which also helps.
...
The screen is up scaled by defualt from 256 to 320 pixels with either crude nearest sample or bilinear (the x axis up scaling can be turned on and off in the in-game menu by pressing select + left). Due to some coding tricks bilinaer is just as fast as the more crude nearest sample but bilinear looks much better. Y axis is not scaled at all.
...
There are options to use DMA but in its current form there is no speed benefit over using the second CPU. ROMs can be put on the SD card (formatted as FAT) with 8.3 file names and then mapped to the file menu with the "roms.txt" file that also needs to be in the root dir of the SD card.
...
only a verify (no write) will take place to make sure the data is intact on the ESP32 flash memory.

## Hack
Real NES hardware can only display 8 (out of 64) sprites on a scanline. Usually games cycle the sprites to avoid them to be completly invisible but this has the drawback of making them flicker. I added a hack to allow the emulator to render potentially all 64 sprites on a scanline and avoid the flicker/invisible issue in the config.h file.
Most games will have no problem with this but if prefered return it to 8 to make it run like real NES hardware.

## Controllers
The code base supports different types of controllers but only one at the time. Supported ones are NES, SNES, PSX or raw GPIO buttons (last two are not tested by me). NES and SNES readout timing can be changed to comply with some slower 3rd-party controllers if they behave erratic. Select the one you want in the config.h file

## Compiling
Use the Arduino IDE framework to compile and upload to the ESP32 (tested with V1.8.19 and ESP lib V1.04 newer versions might not work)
