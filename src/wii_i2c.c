...
static const uint8_t data_init3[]     = { 0xfe, 0x03 };
...
static uint8_t shared_read_data[8];
static volatile uint8_t shared_copy_data[8];
...
static esp_err_t wii_i2c_setup_i2c(i2c_port_t i2c_port_num, int sda_pin, int scl_pin)
{
  ...
  i2c_driver_delete(wii_i2c_port_num);
  periph_module_reset(wii_i2c_port_num == 0 ? PERIPH_I2C0_MODULE : PERIPH_I2C1_MODULE);
...
{
  esp_err_t ret;
  if ((ret = wii_i2c_setup_i2c(i2c_port_num, sda_pin, scl_pin)) != ESP_OK) { printf("wii_i2c: setup_i2c failed: %d\n", ret); return 1; }
  if ((ret = wii_i2c_write(data_init1, sizeof(data_init1))) != ESP_OK) { printf("wii_i2c: init1 failed: %d\n", ret); return 1; }
  if ((ret = wii_i2c_write(data_init2, sizeof(data_init2))) != ESP_OK) { printf("wii_i2c: init2 failed: %d\n", ret); return 1; }
  if ((ret = wii_i2c_write(data_init3, sizeof(data_init3))) != ESP_OK) { printf("wii_i2c: init3 failed: %d\n", ret); return 1; }
  return 0;
}
...
void wii_i2c_decode_nesmini(const unsigned char *data, struct wii_i2c_nesmini_state *state)
{
  if (! data) {
    memset(state, 0, sizeof(*state));
    return;
  ...
  state->left  = (data[7] & 0x02) ? 0 : 1;
  state->right = (data[6] & 0x80) ? 0 : 1;
  state->up    = (data[7] & 0x01) ? 0 : 1;
  state->down  = (data[6] & 0x40) ? 0 : 1;
  state->a     = (data[7] & 0x10) ? 0 : 1;
  state->b     = (data[7] & 0x40) ? 0 : 1;
  state->start  = (data[6] & 0x04) ? 0 : 1;
  state->select = (data[6] & 0x10) ? 0 : 1;
}
